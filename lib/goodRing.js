export default function goodRing(start, n_Nodes) {
  // 노드(자연수)의 개수
  // const n_Nodes = 35;
  ...
  let adjMtrx = new Array(n_Nodes + 1)
    .fill(0)
    .map(() => new Array(n_Nodes).fill(0));
  let adjList = new Array(n_Nodes + 1).fill(null).map(() => []);
  ...
  function searchHamilton(circuit) {
    const n = circuit.length - 1;

    // 모든 자연수를 이어 해밀턴 경로를 완성했을 경우 회로인지 확인
    if (n === n_Nodes - 1) {
      // 경로의 끝점과 시작점이 연결되어 있다면 <=> 회로라면
      if (adjMtrx[circuit[n]][circuit[0]] === 1) {
        return circuit; // 현재 탐색한 해밀턴 회로 반환
      }
      // 회로가 아니라면 null 반환
      else {
        return null;
    ...
    for (let i = 0; i < adjList[circuit[n]].length; i++) {
      // 만약 연결된 노드가 이미 경로 안에 포함되지 않았다면 -> 재귀호출로 탐색
      const nextNode = adjList[circuit[n]][i];
      if (!circuit.includes(nextNode)) {
        const res = searchHamilton(circuit.concat([nextNode]));
